...
def main(params):
    ...
    with open("/root/autodl-tmp/pykt-toolkit-main/examples/configs/kt_config.json") as f:
        config = json.load(f)
        train_config = config["train_config"]
        if model_name in ["dkvmn","deep_irt", "sakt", "desakt","saint","ddsaint","saint++", "akt","ddakt","folibikt", "atkt", "lpkt", "skvmn", "dimkt"]:
            train_config["batch_size"] = 64 ## because of OOM
        if model_name in ["simplekt", "ddsimplekt","bakt_time", "sparsekt"]:
            train_config["batch_size"] = 64 ## because of OOM
    ...
    with open("/root/autodl-tmp/pykt-toolkit-main/examples/configs/data_config.json") as fin:
    ...
    if model_name in ["saint","saint++", "ddsaint","desakt","sakt", "atdkt", "simplekt", "ddsimplekt","bakt_time","folibikt"]:
