...
with open("/root/autodl-tmp/pykt-toolkit-main/examples/configs/wandb.json") as fin:
...
def main(params):
    ...
    with open(os.path.join(save_dir, "config.json")) as fin:
        ...
        model_name, dataset_name, emb_type, seed = trained_params["model_name"], trained_params["dataset_name"], trained_params["emb_type"], trained_params["seed"]
        if model_name in ["saint", "desaint","sakt","desakt", "atdkt"]:
