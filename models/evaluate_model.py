    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["atdkt"]:
                ...
                y = model(dcur)
                y = y[:,1:]
            elif model_name in ["desimplekt"]:
                y = model(dcur)
                y = y[:,1:]
            elif model_name in ["sakt"]:
                y = model(c.long(), r.long(), cshft.long())
            elif model_name in ["desakt"]:
                y, _ = model(c.long(), r.long(), cshft.long(), True)
            elif model_name in ["saint"]:
                y = model(cq.long(), cc.long(), r.long())
                y = y[:, 1:]
            elif model_name in ["ddsaint"]:
                y, _ = model(cq.long(), cc.long(), r.long(), True)
                y = y[:, 1:]
            elif model_name in ["akt"]:                                
                y, reg_loss = model(cc.long(), cr.long(), cq.long())
                y = y[:,1:]
            elif model_name in ["ddakt"]:                                
                y, reg_loss, auxloss = model(cc.long(), cr.long(), cq.long(), True)
                y = y[:,1:]
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "ddakt","folibikt","dtransformer","simplekt", "bakt_time", "sparsekt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
        output = model.out(curhs[0]).squeeze(-1)
        m = nn.Sigmoid()
        p = m(output)
    elif model_name in ["desimplekt"]:
        concat_q = torch.cat([curhs[0], curhs[0]], dim=-1)
        output = model.out(concat_q).squeeze(-1)
        m = nn.Sigmoid()
        p = m(output)
    elif model_name in ["saint","ddsaint"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt","ddakt", "folibikt", "dtransformer", "simplekt", "desimplekt","bakt_time", "sparsekt", "saint","ddsaint","sakt", "desakt","hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt", "ddakt","folibikt","simplekt", "desimplekt","bakt_time", "sparsekt", "saint", "ddsaint","sakt", "desakt","hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt","ddakt","folibikt", "simplekt", "de simple k t","bakt_time", "sparsekt", "saint", "ddsaint","sakt", "desakt","hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    if save_path != "":
        fout = open(save_path, "w", encoding="utf8")
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            elif model_name in ["simplekt","desimplekt", "sparsekt"]:
                y, h = model(dcurori, qtest=True, train=False)
                y = y[:,1:]
            elif model_name in ["akt", "ddakt","folibikt","akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def predict_each_group2(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    while bidx < finalcs.shape[0]:
        ...
        if model_name in ["atdkt"]:
            ...
            y, _ = model(curc.long(), curr.long(), curcshft.long(), True)
        elif model_name in ["saint","ddsaint"]:
            y = model(ccq.long(), ccc.long(), curr.long())
            y = y[:, 1:]
        elif model_name in ["akt","ddakt","folibikt", "cakt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:                                
