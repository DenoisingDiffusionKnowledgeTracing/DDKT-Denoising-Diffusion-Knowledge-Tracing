...
class ddsimplekt(nn.Module):
    def __init__(self, n_question, n_pid, 
            ...
            timesteps=200, beta_start=0.0001, beta_end=0.02, diffusion_w=2.0, beta_sche='exp', delta=1e-3, tau=1, hard=True):
        super().__init__()
        """
        Input:
       ...
       # hyper-parameters for dam
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=self.num_attn_heads, dropout=dropout,
                                    d_model=d_model, d_feature=d_model // num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, seq_len=seq_len,lamb=lamb, tau=self.tau, hard=self.hard)
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if train:
            # Only compute diffusion loss during training
            batch_size = d_output.shape[0]
            t = torch.randint(0, self.timesteps, (batch_size,), device=d_output.device)
            ...
            auxloss = self.lamb * mask_L1 + self.delta * diff_loss
            return preds, y2, y3, auxloss
        else: # Inference
            if qtest:
                return preds, d_output
            else:
                return preds

class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
                 d_ff, n_heads, dropout, kq_same, model_type, seq_len,lamb, tau, hard):
        super().__init__()
        """
        ...
        if model_type in {'ddsimplekt'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, seq_len=seq_len,kq_same=kq_same, lamb=lamb, tau=tau, hard=hard)
    ...
    def forward(self, q_embed_data, qa_embed_data,apply_diffusion=False, diffusion_fn=None):
        ...
        for block in self.blocks_1:
            x, mask_L1 = block(mask=0, query=x, key=x, values=y, apply_pos=True) # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
        ...
        for block in self.blocks_2:
            if flag_first:  # peek current question
                x, mask_L1= block(mask=1, query=x, key=x,values=x, apply_pos=False) # False: 没有FFN, 第一层只有self attention, 对应于xt^
                flag_first = False
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 d_ff, n_heads, dropout,  seq_len, kq_same, lamb, tau, hard):
        super().__init__()
        """
        ...
        self.init_alphas = nn.Parameter(1e-3 * torch.randn(self.n_heads, self.seq_len, self.seq_len))
        self.lamb = lamb
        self.tau = tau
        self.hard = hard
    ...
    def forward(self, mask, query, key, values, apply_pos=True):
        ...
        dam = self.generate_differentiable_attention_mask(seqlen, self.tau, self.hard)
        nopeek_mask = np.triu(
            np.ones((1, 1, seqlen, seqlen)), k=mask).astype('uint8')
        src_mask = (torch.from_numpy(nopeek_mask) == 0).to(device)
        src_mask = src_mask.masked_fill(dam.to(src_mask.device) == 0, False)
