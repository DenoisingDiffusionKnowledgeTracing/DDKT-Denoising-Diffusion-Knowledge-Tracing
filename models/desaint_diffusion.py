...
class ddsaint(nn.Module):
    def __init__(self, num_q, num_c, seq_len, emb_size, num_attn_heads, dropout, n_blocks=1, emb_type="qid", emb_path="", pretrain_dim=768,
                 lamb=1e-4,timesteps=200, beta_start=0.0001, beta_end=0.02, diffusion_w=2.0, beta_sche='exp',delta=1e-3, tau=1, hard=True):
        super().__init__()
        if num_q == num_c and num_q == 0:
            assert num_q != 0
        self.num_q = num_q
        self.num_c = num_c
        self.model_name = "ddsaint"
        ...
        self.decoder = get_clones(Decoder_block(emb_size, 2, num_attn_heads, seq_len, dropout, tau=self.tau, hard=self.hard), self.num_de)
    ...
    def unconditional_denoise(self, x_noisy, t):
        ...
        h = h0.unsqueeze(1).expand(-1, seq_len, -1)
        t_emb = self.time_mlp(t).unsqueeze(1).expand(-1, seq_len, -1)
    ...
    def forward(self, in_ex, in_cat, in_res, qtest=False):
        ...
        for i in range(self.num_en):
            ...
            in_cat = in_ex
        encoder_side = in_ex
        en_out_for_decoder = encoder_side
        diff_loss = 0

        # Using diffusion to enhance representations
        if not qtest:
            # Training phase: calculate diffusion loss
            batch_size = encoder_side.shape[0]
            t = torch.randint(0, self.timesteps, (batch_size,), device=encoder_side.device)
            diff_loss, _ = self.diffusion.p_losses(
                denoise_model=self.denoise,
                x_start=encoder_side,
                h=encoder_side,
                t=t
            )
        else:
            # Inference phase: use diffusion to enhance representation
            en_out_for_decoder = self.diffusion.sample(
                model_forward=self.denoise,
                model_forward_uncon=self.unconditional_denoise,
                h=encoder_side
            )
        ...
        for i in range(self.num_de):
            if i >= 1:
                first_block = False
            in_res,mask_L1 = self.decoder[i](in_res, in_pos, en_out=en_out_for_decoder, first_block=first_block)
            if not first_block:
        ...
        decoder_out = in_res
        # Regular prediction
        res = self.out(self.dropout(decoder_out))
        res = torch.sigmoid(res).squeeze(-1)

        if not qtest:
            auxloss = self.lamb * mask_L1_total + self.delta * diff_loss
            return res, auxloss
        else:
...
class Encoder_block(nn.Module):
    ...
    def forward(self, in_ex, in_cat, in_pos, first_block=True):
        ...
        out, attn_wt = self.multi_en(out, out, out, attn_mask=ut_mask(seq_len=n).to(out.device))
...
class Decoder_block(nn.Module):
    ...
    def __init__(self, dim_model, total_res, heads_de, seq_len, dropout, tau=1, hard=True):
        ...
        self.init_alphas = nn.Parameter(1e-3 * torch.randn(self.heads_de, self.seq_len, self.seq_len))
        self.tau = tau
        self.hard = hard
        
    def generate_differentiable_attention_mask(self,seq_len, tau, hard):

        logits = self.init_alphas[:, :seq_len, :seq_len]  # Adjust logits size based on seq_len
        eps = 1e-5

        G1 = -(torch.empty_like(logits).exponential_() + eps).log()
        G2 = -(torch.empty_like(logits).exponential_() + eps).log()

        gumbels = (logits + (G1-G2)) / tau  # ~Gumbel(logits,tau)
        ...
        causal_mask = torch.tril(torch.ones(seq_len, seq_len, device=logits.device),diagonal=-1)
        dam = mask * causal_mask.unsqueeze(0)
        # print("dam mean:", dam.mean().item(), "dam std:", dam.std().item())
    ...
    def forward(self, in_res, in_pos, en_out,first_block=True):
        ...
        #bmask = 0 if first_block else 1
        dam = self.generate_differentiable_attention_mask(seq_len=n, tau=self.tau, hard=self.hard)  # Generate differentiable attention mask
        
        causal_mask = ut_mask(seq_len=n).to(out.device)
        causal_mask = causal_mask.unsqueeze(0).repeat(self.heads_de, 1, 1)
        causal_mask = causal_mask.float()
        attn_mask = causal_mask - 100.0 * (1.0-dam) 
        attn_mask = attn_mask.repeat(bs, 1, 1)
        attn_mask[:,0,0] = 0
        
        out, attn_wt = self.multi_de1(out, out, out, 
                                     attn_mask=attn_mask) # attention mask upper triangular
        out = self.dropout1(out)
