...
class ddakt(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, seq_len=200, lamb=1e-4,
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout, lamb=lamb, seq_len=seq_len,
                                    d_model=d_model, d_feature=d_model / num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, emb_type=self.emb_type, tau=self.tau, hard=self.hard)
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        if not qtest:
            ...
            return preds, c_reg_loss, auxloss
        else:
            return preds, c_reg_loss, concat_q

class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
                 d_ff, n_heads, dropout, kq_same, model_type, emb_type,seq_len,lamb, tau, hard):
        super().__init__()
        """
        ...
        if model_type in {'ddakt'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same, emb_type=emb_type,seq_len=seq_len,lamb=lamb, tau=tau, hard=hard)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 d_ff, n_heads, dropout,  kq_same, emb_type, seq_len, lamb, tau, hard):
        super().__init__()
        """
        ...
        self.seq_len = seq_len
        self.init_alphas = nn.Parameter(1e-3 * torch.randn(self.num_attention_heads, self.seq_len, self.seq_len))
        self.lamb = lamb
        self.tau = tau
        self.hard = hard
    ...
    def forward(self, mask, query, key, values, apply_pos=True, pdiff=None):
        ...
        dam = self.generate_differentiable_attention_mask(seqlen, self.tau, self.hard) # generate differentiable attention mask, tri=True表示可以看到当前的response
        nopeek_mask = np.triu(
