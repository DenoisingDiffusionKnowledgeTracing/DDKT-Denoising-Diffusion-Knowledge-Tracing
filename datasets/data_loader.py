...
class KTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if "flipmask" in self.dori:
            dcur["flipmask"] = self.dori["flipmask"][index]
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        if "flipmask" in pd.read_csv(sequence_path, nrows=1).columns:
            dori["flipmask"] = []
        ...
        for i, row in df.iterrows():
            ...
            if "flipmask" in dori:
                dori["flipmask"].append(row["flipmask"])
        ...
        for key in dori:
            if key not in ["rseqs", "flipmask"]:#in ["smasks", "tseqs"]:
                dori[key] = LongTensor(dori[key])
            elif key == "rseqs":
                dori[key] = FloatTensor(dori[key])
            elif key == "flipmask":
                dori[key] = dori[key] # Keep as list of strings
